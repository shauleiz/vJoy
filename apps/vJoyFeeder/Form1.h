...
#pragma region Manually generated code
    ...
    private:
        ...
        long AxisWheelmax, AxisWheelmin;
        long AxisAcceleratormax, AxisAcceleratormin;
        long AxisBrakemax, AxisBrakemin;
        long AxisClutchmax, AxisClutchmin;
        long AxisSteeringmax, AxisSteeringmin;
        long AxisAileronmax, AxisAileronmin;
        long AxisRuddermax, AxisRuddermin;
        long AxisThrottlemax, AxisThrottlemin;
    ...
    private: System::Windows::Forms::TextBox^ textBoxWheel;
    private: System::Windows::Forms::CheckBox^ checkBoxEnableFeeding;
    private: System::Windows::Forms::GroupBox^ groupBoxThrottle;

    private: System::Windows::Forms::TrackBar^ trackBarThrottle;
    private: System::Windows::Forms::TextBox^ textBoxThrottle;
    private: System::Windows::Forms::GroupBox^ groupBoxRudder;



    private: System::Windows::Forms::TrackBar^ trackBarRudder;
    private: System::Windows::Forms::TextBox^ textBoxRudder;
    private: System::Windows::Forms::GroupBox^ groupBoxAileron;



    private: System::Windows::Forms::TrackBar^ trackBarAileron;
    private: System::Windows::Forms::TextBox^ textBoxAileron;
    private: System::Windows::Forms::GroupBox^ groupBoxSteering;



    private: System::Windows::Forms::TrackBar^ trackBarSteering;
    private: System::Windows::Forms::TextBox^ textBoxSteering;
    private: System::Windows::Forms::GroupBox^ groupBoxClutch;



    private: System::Windows::Forms::TrackBar^ trackBarClutch;
    private: System::Windows::Forms::TextBox^ textBoxClutch;
    ...
    private: System::Windows::Forms::TextBox^ textBoxBrake;
    private: System::Windows::Forms::GroupBox^ groupBoxAccelerator;


    private: System::Windows::Forms::TrackBar^ trackBarAccelerator;

    private: System::Windows::Forms::TextBox^ textBoxAccelerator;
    ...
    private: System::Void ValidControls(void) {
        ...
        */
        ...
        groupBoxWheel->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_WHL) == TRUE;
        groupBoxAccelerator->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_ACCELERATOR) == TRUE;
        groupBoxBrake->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_BRAKE) == TRUE;
        groupBoxClutch->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_CLUTCH) == TRUE;
        groupBoxSteering->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_STEERING) == TRUE;
        groupBoxAileron->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_AILERON) == TRUE;
        groupBoxRudder->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_RUDDER) == TRUE;
        groupBoxThrottle->Enabled = GetVJDAxisExist(ReportId, HID_USAGE_THROTTLE) == TRUE;
        ...
        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_WHL, &max) && GetVJDAxisMin(ReportId, HID_USAGE_WHL, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarWheel->Value = min;
            textBoxWheel->Text = System::Convert::ToString((int)trackBarWheel->Value);
            AxisWheelmax = max;
            AxisWheelmin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_ACCELERATOR, &max) && GetVJDAxisMin(ReportId, HID_USAGE_ACCELERATOR, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarAccelerator->Value = min;
            textBoxAccelerator->Text = System::Convert::ToString((int)trackBarAccelerator->Value);
            AxisAcceleratormax = max;
            AxisAcceleratormin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_BRAKE, &max) && GetVJDAxisMin(ReportId, HID_USAGE_BRAKE, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarBrake->Value = min;
            textBoxBrake->Text = System::Convert::ToString((int)trackBarBrake->Value);
            AxisBrakemax = max;
            AxisBrakemin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_CLUTCH, &max) && GetVJDAxisMin(ReportId, HID_USAGE_CLUTCH, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarClutch->Value = min;
            textBoxClutch->Text = System::Convert::ToString((int)trackBarClutch->Value);
            AxisClutchmax = max;
            AxisClutchmin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_STEERING, &max) && GetVJDAxisMin(ReportId, HID_USAGE_STEERING, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarSteering->Value = min;
            textBoxSteering->Text = System::Convert::ToString((int)trackBarSteering->Value);
            AxisSteeringmax = max;
            AxisSteeringmin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_AILERON, &max) && GetVJDAxisMin(ReportId, HID_USAGE_AILERON, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarAileron->Value = min;
            textBoxAileron->Text = System::Convert::ToString((int)trackBarAileron->Value);
            AxisAileronmax = max;
            AxisAileronmin = min;
        };

        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_RUDDER, &max) && GetVJDAxisMin(ReportId, HID_USAGE_RUDDER, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarRudder->Value = min;
            textBoxRudder->Text = System::Convert::ToString((int)trackBarRudder->Value);
            AxisRuddermax = max;
            AxisRuddermin = min;
        };
        ...
        if (max < 0 || min < 0)
            GetVJDAxisMax(ReportId, HID_USAGE_THROTTLE, &max) && GetVJDAxisMin(ReportId, HID_USAGE_THROTTLE, &min);
        if ((max >= 0) && (min >= 0)) {
            trackBarThrottle->Value = min;
            textBoxThrottle->Text = System::Convert::ToString((int)trackBarThrottle->Value);
            AxisThrottlemax = max;
            AxisThrottlemin = min;
        };
        ...
        delete str;
           ...
           // Read from vJoy and update internal values
    private: System::Void update_from_vjoy(void) {
        ...
        // Axes
        this->trackBarX->Value = ConvPercentage(Report.wAxisX, AxisXmax, AxisXmin);
        this->trackBarY->Value = ConvPercentage(Report.wAxisY, AxisYmax, AxisYmin);
        this->trackBarZ->Value = ConvPercentage(Report.wAxisZ, AxisZmax, AxisZmin);
        this->trackBarRx->Value = ConvPercentage(Report.wAxisXRot, AxisRXmax, AxisRXmin);
        this->trackBarRy->Value = ConvPercentage(Report.wAxisYRot, AxisRYmax, AxisRYmin);
        this->trackBarRz->Value = ConvPercentage(Report.wAxisZRot, AxisRZmax, AxisRZmin);
        this->trackBarSL0->Value = ConvPercentage(Report.wSlider, AxisSL0max, AxisSL0min);
        this->trackBarSL1->Value = ConvPercentage(Report.wDial, AxisSL1max, AxisSL1min);

        this->trackBarWheel->Value = ConvPercentage(Report.wWheel, AxisWheelmax, AxisWheelmin);
        this->trackBarAccelerator->Value = ConvPercentage(Report.wAccelerator, AxisAcceleratormax, AxisAcceleratormin);
        this->trackBarBrake->Value = ConvPercentage(Report.wBrake, AxisBrakemax, AxisBrakemin);
        this->trackBarClutch->Value = ConvPercentage(Report.wClutch, AxisClutchmax, AxisClutchmin);
        this->trackBarSteering->Value = ConvPercentage(Report.wSteering, AxisSteeringmax, AxisSteeringmin);
        this->trackBarAileron->Value = ConvPercentage(Report.wAileron, AxisAileronmax, AxisAileronmin);
        this->trackBarRudder->Value = ConvPercentage(Report.wRudder, AxisRuddermax, AxisRuddermin);
        this->trackBarThrottle->Value = ConvPercentage(Report.wThrottle, AxisThrottlemax, AxisThrottlemin);

        // Buttons
        for (int index = 0; index<VJOY_NUMBER_OF_BUTTONS; index++) {
            LONG* Target;
            int Shift = index%32;
            ...
            BOOL active = (*Target & mask);
            if (active!=0) {
                this->CBvJoyButton[index]->Checked = true;
            } else {
                this->CBvJoyButton[index]->Checked = false;
            }
        }
    ...
    private: System::Void update_to_vjoy(void) {
        JOYSTICK_POSITION	Report;
        static UINT pov[4] = { (UINT)-1,(UINT)-1,(UINT)-1,(UINT)-1 };
...
#pragma region Windows Form Designer generated code
           ...
           void InitializeComponent(void)
           {
               ...
               this->groupBoxThrottle = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarThrottle = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxThrottle = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxRudder = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarRudder = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxRudder = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxAileron = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarAileron = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxAileron = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxSteering = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarSteering = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxSteering = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxClutch = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarClutch = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxClutch = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxBrake = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarBrake = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxBrake = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxAccelerator = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarAccelerator = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxAccelerator = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxWheel = (gcnew System::Windows::Forms::GroupBox());
               this->trackBarWheel = (gcnew System::Windows::Forms::TrackBar());
               this->textBoxWheel = (gcnew System::Windows::Forms::TextBox());
               this->groupBoxButtons = (gcnew System::Windows::Forms::GroupBox());
               this->groupBoxTarget = (gcnew System::Windows::Forms::GroupBox());
               this->checkBoxEnableFeeding = (gcnew System::Windows::Forms::CheckBox());
               ...
               this->groupBoxThrottle->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarThrottle))->BeginInit();
               this->groupBoxRudder->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarRudder))->BeginInit();
               this->groupBoxAileron->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarAileron))->BeginInit();
               this->groupBoxSteering->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarSteering))->BeginInit();
               this->groupBoxClutch->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarClutch))->BeginInit();
               this->groupBoxBrake->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarBrake))->BeginInit();
               this->groupBoxAccelerator->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarAccelerator))->BeginInit();
               this->groupBoxWheel->SuspendLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarWheel))->BeginInit();
               ...
               this->textBoxInfo->Size = System::Drawing::Size(521, 43);
               ...
               this->groupBoxPov1->Location = System::Drawing::Point(406, 407);
               ...
               this->groupBoxKnobPov1->Location = System::Drawing::Point(406, 407);
               this->groupBoxKnobPov1->Name = L"groupBoxKnobPov1";
               this->groupBoxKnobPov1->Size = System::Drawing::Size(134, 192);
               ...
               this->groupBoxAxes->Controls->Add(this->groupBoxThrottle);
               this->groupBoxAxes->Controls->Add(this->groupBoxRudder);
               this->groupBoxAxes->Controls->Add(this->groupBoxAileron);
               this->groupBoxAxes->Controls->Add(this->groupBoxSteering);
               this->groupBoxAxes->Controls->Add(this->groupBoxClutch);
               this->groupBoxAxes->Controls->Add(this->groupBoxBrake);
               this->groupBoxAxes->Controls->Add(this->groupBoxAccelerator);
               this->groupBoxAxes->Controls->Add(this->groupBoxWheel);
               ...
               this->groupBoxAxes->Size = System::Drawing::Size(1058, 338);
               ...
               // groupBoxThrottle
               // 
               this->groupBoxThrottle->Controls->Add(this->trackBarThrottle);
               this->groupBoxThrottle->Controls->Add(this->textBoxThrottle);
               this->groupBoxThrottle->Location = System::Drawing::Point(992, 16);
               this->groupBoxThrottle->Name = L"groupBoxThrottle";
               this->groupBoxThrottle->Size = System::Drawing::Size(60, 310);
               this->groupBoxThrottle->TabIndex = 10;
               this->groupBoxThrottle->TabStop = false;
               this->groupBoxThrottle->Text = L"Throttle";
               // 
               // trackBarThrottle
               // 
               this->trackBarThrottle->LargeChange = 1;
               this->trackBarThrottle->Location = System::Drawing::Point(6, 45);
               this->trackBarThrottle->Maximum = 100;
               this->trackBarThrottle->Name = L"trackBarThrottle";
               this->trackBarThrottle->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarThrottle->Size = System::Drawing::Size(45, 260);
               ...
               this->trackBarThrottle->Scroll += gcnew System::EventHandler(this, &Form1::trackBarThrottle_Scroll);
               // 
               // textBoxThrottle
               // 
               this->textBoxThrottle->Location = System::Drawing::Point(6, 19);
               this->textBoxThrottle->Name = L"textBoxThrottle";
               ...
               // groupBoxRudder
               // 
               this->groupBoxRudder->Controls->Add(this->trackBarRudder);
               this->groupBoxRudder->Controls->Add(this->textBoxRudder);
               this->groupBoxRudder->Location = System::Drawing::Point(926, 16);
               this->groupBoxRudder->Name = L"groupBoxRudder";
               this->groupBoxRudder->Size = System::Drawing::Size(60, 310);
               this->groupBoxRudder->TabIndex = 9;
               this->groupBoxRudder->TabStop = false;
               this->groupBoxRudder->Text = L"Rudder";
               // 
               // trackBarRudder
               // 
               this->trackBarRudder->LargeChange = 1;
               this->trackBarRudder->Location = System::Drawing::Point(6, 45);
               this->trackBarRudder->Maximum = 100;
               this->trackBarRudder->Name = L"trackBarRudder";
               this->trackBarRudder->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarRudder->Size = System::Drawing::Size(45, 260);
               this->trackBarRudder->TabIndex = 2;
               this->trackBarRudder->TickFrequency = 10;
               this->trackBarRudder->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarRudder->Scroll += gcnew System::EventHandler(this, &Form1::trackBarRudder_Scroll);
               // 
               // textBoxRudder
               // 
               this->textBoxRudder->Location = System::Drawing::Point(6, 19);
               this->textBoxRudder->Name = L"textBoxRudder";
               ...
               // groupBoxAileron
               // 
               this->groupBoxAileron->Controls->Add(this->trackBarAileron);
               this->groupBoxAileron->Controls->Add(this->textBoxAileron);
               this->groupBoxAileron->Location = System::Drawing::Point(860, 16);
               this->groupBoxAileron->Name = L"groupBoxAileron";
               this->groupBoxAileron->Size = System::Drawing::Size(60, 310);
               this->groupBoxAileron->TabIndex = 8;
               this->groupBoxAileron->TabStop = false;
               this->groupBoxAileron->Text = L"Aileron";
               // 
               // trackBarAileron
               // 
               this->trackBarAileron->LargeChange = 1;
               this->trackBarAileron->Location = System::Drawing::Point(6, 45);
               this->trackBarAileron->Maximum = 100;
               this->trackBarAileron->Name = L"trackBarAileron";
               this->trackBarAileron->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarAileron->Size = System::Drawing::Size(45, 260);
               this->trackBarAileron->TabIndex = 2;
               this->trackBarAileron->TickFrequency = 10;
               this->trackBarAileron->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarAileron->Scroll += gcnew System::EventHandler(this, &Form1::trackBarAileron_Scroll);
               // 
               // textBoxAileron
               // 
               this->textBoxAileron->Location = System::Drawing::Point(6, 19);
               this->textBoxAileron->Name = L"textBoxAileron";
               ...
               // groupBoxSteering
               // 
               this->groupBoxSteering->Controls->Add(this->trackBarSteering);
               this->groupBoxSteering->Controls->Add(this->textBoxSteering);
               this->groupBoxSteering->Location = System::Drawing::Point(794, 16);
               this->groupBoxSteering->Name = L"groupBoxSteering";
               this->groupBoxSteering->Size = System::Drawing::Size(60, 310);
               this->groupBoxSteering->TabIndex = 7;
               this->groupBoxSteering->TabStop = false;
               this->groupBoxSteering->Text = L"Steering";
               // 
               // trackBarSteering
               // 
               this->trackBarSteering->LargeChange = 1;
               this->trackBarSteering->Location = System::Drawing::Point(6, 45);
               this->trackBarSteering->Maximum = 100;
               this->trackBarSteering->Name = L"trackBarSteering";
               this->trackBarSteering->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarSteering->Size = System::Drawing::Size(45, 260);
               this->trackBarSteering->TabIndex = 2;
               this->trackBarSteering->TickFrequency = 10;
               this->trackBarSteering->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarSteering->Scroll += gcnew System::EventHandler(this, &Form1::trackBarSteering_Scroll);
               // 
               // textBoxSteering
               // 
               this->textBoxSteering->Location = System::Drawing::Point(6, 19);
               this->textBoxSteering->Name = L"textBoxSteering";
               ...
               // groupBoxClutch
               // 
               this->groupBoxClutch->Controls->Add(this->trackBarClutch);
               this->groupBoxClutch->Controls->Add(this->textBoxClutch);
               this->groupBoxClutch->Location = System::Drawing::Point(728, 16);
               this->groupBoxClutch->Name = L"groupBoxClutch";
               this->groupBoxClutch->Size = System::Drawing::Size(60, 310);
               this->groupBoxClutch->TabIndex = 6;
               this->groupBoxClutch->TabStop = false;
               this->groupBoxClutch->Text = L"Cltuch";
               // 
               // trackBarClutch
               // 
               this->trackBarClutch->LargeChange = 1;
               this->trackBarClutch->Location = System::Drawing::Point(6, 45);
               this->trackBarClutch->Maximum = 100;
               this->trackBarClutch->Name = L"trackBarClutch";
               this->trackBarClutch->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarClutch->Size = System::Drawing::Size(45, 260);
               this->trackBarClutch->TabIndex = 2;
               this->trackBarClutch->TickFrequency = 10;
               this->trackBarClutch->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarClutch->Scroll += gcnew System::EventHandler(this, &Form1::trackBarClutch_Scroll);
               // 
               // textBoxClutch
               // 
               this->textBoxClutch->Location = System::Drawing::Point(6, 19);
               this->textBoxClutch->Name = L"textBoxClutch";
               ...
               // groupBoxBrake
               // 
               this->groupBoxBrake->Controls->Add(this->trackBarBrake);
               this->groupBoxBrake->Controls->Add(this->textBoxBrake);
               this->groupBoxBrake->Location = System::Drawing::Point(662, 16);
               this->groupBoxBrake->Name = L"groupBoxBrake";
               this->groupBoxBrake->Size = System::Drawing::Size(60, 310);
               this->groupBoxBrake->TabIndex = 5;
               this->groupBoxBrake->TabStop = false;
               this->groupBoxBrake->Text = L"Brake";
               // 
               // trackBarBrake
               // 
               this->trackBarBrake->LargeChange = 1;
               this->trackBarBrake->Location = System::Drawing::Point(6, 45);
               this->trackBarBrake->Maximum = 100;
               this->trackBarBrake->Name = L"trackBarBrake";
               this->trackBarBrake->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarBrake->Size = System::Drawing::Size(45, 260);
               this->trackBarBrake->TabIndex = 2;
               this->trackBarBrake->TickFrequency = 10;
               this->trackBarBrake->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarBrake->Scroll += gcnew System::EventHandler(this, &Form1::trackBarBrake_Scroll);
               // 
               // textBoxBrake
               // 
               this->textBoxBrake->Location = System::Drawing::Point(6, 19);
               this->textBoxBrake->Name = L"textBoxBrake";
               ...
               // groupBoxAccelerator
               // 
               this->groupBoxAccelerator->Controls->Add(this->trackBarAccelerator);
               this->groupBoxAccelerator->Controls->Add(this->textBoxAccelerator);
               this->groupBoxAccelerator->Location = System::Drawing::Point(596, 16);
               this->groupBoxAccelerator->Name = L"groupBoxAccelerator";
               this->groupBoxAccelerator->Size = System::Drawing::Size(60, 310);
               this->groupBoxAccelerator->TabIndex = 4;
               this->groupBoxAccelerator->TabStop = false;
               this->groupBoxAccelerator->Text = L"Accel";
               // 
               // trackBarAccelerator
               // 
               this->trackBarAccelerator->LargeChange = 1;
               this->trackBarAccelerator->Location = System::Drawing::Point(6, 45);
               this->trackBarAccelerator->Maximum = 100;
               this->trackBarAccelerator->Name = L"trackBarAccelerator";
               this->trackBarAccelerator->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarAccelerator->Size = System::Drawing::Size(45, 260);
               this->trackBarAccelerator->TabIndex = 2;
               this->trackBarAccelerator->TickFrequency = 10;
               this->trackBarAccelerator->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarAccelerator->Scroll += gcnew System::EventHandler(this, &Form1::trackBarAccelerator_Scroll);
               // 
               // textBoxAccelerator
               // 
               this->textBoxAccelerator->Location = System::Drawing::Point(6, 19);
               this->textBoxAccelerator->Name = L"textBoxAccelerator";
               this->textBoxAccelerator->Size = System::Drawing::Size(43, 20);
               this->textBoxAccelerator->TabIndex = 1;
               this->textBoxAccelerator->Text = L"0";
               this->textBoxAccelerator->TextChanged += gcnew System::EventHandler(this, &Form1::textBoxAccelerator_TextChanged);
               // 
               // groupBoxWheel
               // 
               this->groupBoxWheel->Controls->Add(this->trackBarWheel);
               this->groupBoxWheel->Controls->Add(this->textBoxWheel);
               this->groupBoxWheel->Location = System::Drawing::Point(530, 16);
               this->groupBoxWheel->Name = L"groupBoxWheel";
               this->groupBoxWheel->Size = System::Drawing::Size(60, 310);
               this->groupBoxWheel->TabIndex = 3;
               this->groupBoxWheel->TabStop = false;
               this->groupBoxWheel->Text = L"Wheel";
               // 
               // trackBarWheel
               // 
               this->trackBarWheel->LargeChange = 1;
               this->trackBarWheel->Location = System::Drawing::Point(6, 45);
               this->trackBarWheel->Maximum = 100;
               this->trackBarWheel->Name = L"trackBarWheel";
               this->trackBarWheel->Orientation = System::Windows::Forms::Orientation::Vertical;
               this->trackBarWheel->Size = System::Drawing::Size(45, 260);
               this->trackBarWheel->TabIndex = 2;
               this->trackBarWheel->TickFrequency = 10;
               this->trackBarWheel->TickStyle = System::Windows::Forms::TickStyle::Both;
               this->trackBarWheel->Scroll += gcnew System::EventHandler(this, &Form1::trackBarWheel_Scroll);
               // 
               // textBoxWheel
               // 
               this->textBoxWheel->Location = System::Drawing::Point(6, 19);
               this->textBoxWheel->Name = L"textBoxWheel";
               this->textBoxWheel->Size = System::Drawing::Size(43, 20);
               this->textBoxWheel->TabIndex = 1;
               this->textBoxWheel->Text = L"0";
               this->textBoxWheel->TextChanged += gcnew System::EventHandler(this, &Form1::textBoxWheel_TextChanged);
               // 
               // groupBoxButtons
               // 
               this->groupBoxButtons->Location = System::Drawing::Point(11, 407);
               this->groupBoxButtons->Name = L"groupBoxButtons";
               this->groupBoxButtons->Size = System::Drawing::Size(380, 217);
               ...
               this->checkBoxEnableFeeding->TabIndex = 6;
               this->checkBoxEnableFeeding->Text = L"Enable Feeding";
               this->checkBoxEnableFeeding->UseVisualStyleBackColor = true;
               this->checkBoxEnableFeeding->Click += gcnew System::EventHandler(this, &Form1::checkBoxEnableFeeding_Click);
               // 
               // listBoxTarget
               // 
               ...
               this->ClientSize = System::Drawing::Size(1076, 627);
               ...
               this->MaximumSize = System::Drawing::Size(1096, 670);
               this->MinimumSize = System::Drawing::Size(1096, 670);
               this->Name = L"Form1";
               ...
               this->groupBoxThrottle->ResumeLayout(false);
               this->groupBoxThrottle->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarThrottle))->EndInit();
               this->groupBoxRudder->ResumeLayout(false);
               this->groupBoxRudder->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarRudder))->EndInit();
               this->groupBoxAileron->ResumeLayout(false);
               this->groupBoxAileron->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarAileron))->EndInit();
               this->groupBoxSteering->ResumeLayout(false);
               this->groupBoxSteering->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarSteering))->EndInit();
               this->groupBoxClutch->ResumeLayout(false);
               this->groupBoxClutch->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarClutch))->EndInit();
               this->groupBoxBrake->ResumeLayout(false);
               this->groupBoxBrake->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarBrake))->EndInit();
               this->groupBoxAccelerator->ResumeLayout(false);
               this->groupBoxAccelerator->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarAccelerator))->EndInit();
               this->groupBoxWheel->ResumeLayout(false);
               this->groupBoxWheel->PerformLayout();
               (cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->trackBarWheel))->EndInit();
...
#pragma region Individual axes and buttons
    ...
    private: System::Void trackBarWheel_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarWheel, this->textBoxWheel);
    }
    private: System::Void trackBarAccelerator_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarAccelerator, this->textBoxAccelerator);
    }
    private: System::Void trackBarBrake_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarBrake, this->textBoxBrake);
    }
    private: System::Void trackBarClutch_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarClutch, this->textBoxClutch);
    }
    private: System::Void trackBarSteering_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarSteering, this->textBoxSteering);
    }
    private: System::Void trackBarAileron_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarAileron, this->textBoxAileron);
    }
    private: System::Void trackBarRudder_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarRudder, this->textBoxRudder);
    }
    private: System::Void trackBarThrottle_Scroll(System::Object^ sender, System::EventArgs^ e) {
        trackBar_Scroll(this->trackBarThrottle, this->textBoxThrottle);
    }
    ...
    private: System::Void textBoxWheel_TextChanged(System::Object^ sender, System::EventArgs^ e) {
        if (this->checkBoxEnableFeeding->Checked) {
            textbox_text_changed(this->trackBarWheel, this->textBoxWheel);
            SetAxis(ConvAbsolute(this->trackBarWheel->Value, AxisWheelmax, AxisWheelmin), System::Convert::ToInt32(listBoxTarget->SelectedItem->ToString()), HID_USAGE_WHL);
        }
    }
    private: System::Void textBoxAccelerator_TextChanged(System::Object^ sender, System::EventArgs^ e) {
        if (this->checkBoxEnableFeeding->Checked) {
            textbox_text_changed(this->trackBarAccelerator, this->textBoxAccelerator);
